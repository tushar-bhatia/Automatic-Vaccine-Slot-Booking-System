...
console.log('\n###############   Welcome to the Automated Slot Booking by Tushar Bhatia  ###############\n');

var mobile = prompt('Enter Your Mobile Number: ');
while(isNaN(mobile) || mobile.length!=10) mobile = prompt('Please enter correct Mobile Number: ');
user.login(mobile)
.then((token) => {
...
function showBeneficiaries() {
    ...
    .then((beneficiaryList) => {
        var yetToReceiveVaccination =  beneficiaryList.filter(function(beneficiary){
            return beneficiary.vaccination_status != constants.VACCINATION_STATUS.VACCINATED;
        });
        if(yetToReceiveVaccination.length == 0) {
            ...
            if(yetToReceiveVaccination.length == 1) {
                ...
                var inputs = prompt('Enter comma separated value of indexes for which you want to book slots: ');
...
function searchForPreferences(beneficiaries) {
    beneficiaries.forEach((beneficiary) => {
        ...
        if(preference == null) {
            ...
            loader.createPreference(beneficiary)
            .then((newPreference)=>{
...
async function checkAndBook(preference) {
    ...
    try {
        ...
        if(sessions.length>0){
            ...
            slot.bookAppointment(sessions[0], preference)
            .then((response)=>{
                console.log(`\nCongratulation!! Appointment Booked successfully with appointment number: ${response}`);
                console.log(`Appointment Details for ${util.getName(preference)} are as follows:`);
                console.table(util.getSessionDetails(sessions[0]));
                console.log(`Do not forget to take ${util.getIDName(preference)} with you!`);
...
function updateAdditionalInfo(preference, beneficiary){
    if(beneficiary['vaccination_status'] == constants.VACCINATION_STATUS.NOT_VACCINATED) preference['dose'] = 1;
    else if(beneficiary['vaccination_status'] == constants.VACCINATION_STATUS.PARTIALLY_VACCINATED) preference['dose'] = 2;
...
function processPreference(beneficiary, preference, processSave) {
    return new Promise((resolve, reject)=>{
        ...
        while(decision!=1 && decision!=2) decision = parseInt(prompt('Please enter correct choice: '));
        if(decision == 1){
            if(processSave) {
                let save = prompt('Do you want to save above preference for future reference (y/n): ');
                while(save.toLowerCase()!='y' && save.toLowerCase()!='n') save = prompt('Please enter correct choice: ');
                if(save.toLowerCase() == 'y') loader.savePreference(beneficiary.beneficiary_reference_id, preference);
            }
